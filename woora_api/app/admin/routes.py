...
def get_agent_commission_setting():
    """Récupère le pourcentage de commission des agents."""
    try:
        commission_setting = AppSetting.query.filter_by(setting_key='agent_commission_percentage').first()
        
        commission_percentage = float(commission_setting.setting_value) if commission_setting else 0.0

        return jsonify({'agent_commission_percentage': commission_percentage}), 200
    except Exception as e:
        current_app.logger.error(f"Erreur lors de la récupération du paramètre de commission agent: {e}", exc_info=True)
        return jsonify({"message": "Erreur interne du serveur."}), 500
...
def update_agent_commission_setting():
    """Met à jour le pourcentage de commission des agents."""
    json_data = request.get_json()
    if not json_data or 'agent_commission_percentage' not in json_data:
        return jsonify({"message": "Données JSON non fournies ou pourcentage de commission manquant."}), 400

    try:
        commission_percentage = float(json_data['agent_commission_percentage'])
        if not (0 <= commission_percentage <= 100):
            return jsonify({"message": "Le pourcentage de commission doit être entre 0 et 100."}), 400

        commission_setting = AppSetting.query.filter_by(setting_key='agent_commission_percentage').first()
        if commission_setting:
            commission_setting.setting_value = str(commission_percentage)
        else:
            commission_setting = AppSetting(
                setting_key='agent_commission_percentage',
                setting_value=str(commission_percentage),
                data_type='decimal',
                description='Pourcentage de commission pour les agents immobiliers.'
            )
            db.session.add(commission_setting)
        
        db.session.commit()
        
        return jsonify({"message": "Pourcentage de commission agent mis à jour avec succès."}), 200

    except ValueError:
        return jsonify({"message": "Le pourcentage de commission doit être un nombre valide."}), 400
    except Exception as e:
        db.session.rollback()
